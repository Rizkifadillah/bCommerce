...
class Controller extends BaseController
{
	...
	protected function rajaOngkirRequest($resource, $params = [], $method = 'GET')
	{
		...
		$headers = ['key' => 'c42e3d26dee2245a824617f138dda0dc'];
		$requestParams = [
			'headers' => $headers,
		];

		$url = 'http://api.rajaongkir.com/starter/' . $resource;
		if ($params && $method == 'POST') {
			$requestParams['form_params'] = $params;
		} else if ($params && $method == 'GET') {
			$query = is_array($params) ? '?'.http_build_query($params) : '';
			$url = 'http://api.rajaongkir.com/starter/' . $resource . $query;
		}
	...
	protected function initPaymentGateway()
	{
		...
		\Midtrans\Config::$serverKey = "SB-Mid-server-kr7YM2jOelI3BfCDa2X1yOxf";
